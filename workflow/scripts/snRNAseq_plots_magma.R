...
GWAS <- c('SCZ', 'HEIGHT')
DATA_DIR <- "~/Desktop/fetal_brain_snRNAseq_110122/results/magma_celltyping/"
...
for (REGION in REGIONS) {
  ...
  for (DISORDER in GWAS) {
    ...
    top10Data <- read.table(paste0(DATA_DIR, DISORDER, NAME_BODY, 
    ...
    if (REGION == "pfc") {
      ...
      top10Plot <- ggplot(data = top10Data, aes(x = -log10(as.numeric(P)), y = factor(VARIABLE, rev(levels(factor(VARIABLE)))))) +
        geom_bar(stat = "identity", fill = c('#3CBB75FF', '#FF5959', '#FF5959', '#CEE5FD', '#3CBB75FF', 
                                             '#3CBB75FF', '#3CBB75FF', '#D078FF', '#CEE5FD', '#FF5959', 
                                             '#CEE5FD', '#CEE5FD', '#DCBEFF', '#3CBB75FF', '#3CBB75FF',
                                             '#3CBB75FF', '#FDE725FF', '#CEE5FD', '#3CBB75FF', '#9A6324',
                                             '#F58231'), color = 'black') +
        geom_vline(xintercept=-log10(0.00054), linetype = "dashed", color = "black") +
        geom_vline(xintercept=-log10(0.05), linetype = "dotted", color = "black") +
        theme_bw() +
      ...
      top10Plot <- ggplot(data = top10Data, aes(x = -log10(as.numeric(P)), y = factor(VARIABLE, rev(levels(factor(VARIABLE)))))) +
        geom_bar(stat = "identity", fill = c('#CEE5FD', '#B200ED', '#FF5959', '#FF5959', '#CEE5FD', 
                                             '#3CBB75FF', '#CEE5FD', '#3CBB75FF', "#DCBEFF", '#3CBB75FF', 
                                             '#B200ED', '#CEE5FD', '#CEE5FD', '#FF5959', '#3CBB75FF',
      ...
      top10Plot <- ggplot(data = top10Data, aes(x = -log10(as.numeric(P)), y = factor(VARIABLE, rev(levels(factor(VARIABLE)))))) +
        geom_bar(stat = "identity", fill = c('#FF5959', '#3CBB75FF', '#FF5959', '#3CBB75FF', '#3CBB75FF', 
                                             '#FF5959', '#3CBB75FF', '#3CBB75FF', '#CEE5FD', '#DCBEFF', 
                                             '#FF5959', '#D078FF', '#FF5959', '#3CBB75FF', '#FF5959',
